...
"""
ربات مدیریت Excel پیشرفته - فایل اصلی کامل
نسخه 2.0

برای اجرای ربات از این فایل استفاده کنید:
python run_bot.py
"""
...
# Import توابع از main1
from main1 import (
    start, add_record_start, add_record_process, show_all_records, 
    send_excel_file, edit_start, edit_row_select, edit_field_select, edit_value_apply
)
...
# توابع از main2 (کپی کرده‌ام برای سادگی)
...
async def delete_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for col in df.columns:
            value = clean_value(deleted_record[col])
            if value:
                record_info += f"• {col}: {value}\n"
        ...
        if create_excel(df, user_theme):
            ...
            logger.info(f"User {update.effective_user.id} deleted a record")
        else:
        ...
        logger.error(f"Error in delete_confirm: {e}")
...
async def search_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard.append([KeyboardButton("🔍 جستجو در همه فیلدها")])
...
async def search_field_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """انتخاب فیلد جستجو"""
    ...
    if field == "🔍 جستجو در همه فیلدها":
        ...
        await update.message.reply_text("🔍 **کلیدواژه جستجو را وارد کنید:**")
        return SEARCH_VALUE
    ...
    await update.message.reply_text(f"🔍 **کلیدواژه جستجو در فیلد '{field}':**")
    return SEARCH_VALUE
...
async def search_process(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        field = context.user_data['search_field']
        ...
        message = format_search_results(results, keyword, MAX_SEARCH_RESULTS)
...
async def field_management_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """شروع مدیریت فیلدها"""
    fields = load_fields()
    
    keyboard = [
        [KeyboardButton("➕ اضافه کردن فیلد"), KeyboardButton("🗑️ حذف فیلد")],
        [KeyboardButton("📋 نمایش فیلدها"), KeyboardButton("🔄 بازگشت به پیش‌فرض")],
        [KeyboardButton("🏠 بازگشت به منوی اصلی")]
    ]
    
    msg = f"⚙️ **مدیریت فیلدها**\n\n"
    msg += f"📊 تعداد فیلدهای فعلی: {len(fields)}\n"
    msg += f"📋 فیلدهای موجود:\n"
...
async def field_management_handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """مدیریت عملیات فیلدها"""
    text = update.message.text
    
    if text == "➕ اضافه کردن فیلد":
        await update.message.reply_text("📝 **نام فیلد جدید را وارد کنید:**")
        return ADD_FIELD
    elif text == "🗑️ حذف فیلد":
        return await delete_field_start(update, context)
    elif text == "📋 نمایش فیلدها":
        return await show_fields(update, context)
    elif text == "🔄 بازگشت به پیش‌فرض":
        return await reset_fields(update, context)
...
async def add_field_process(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if new_field in fields:
            await update.message.reply_text("❌ این فیلد از قبل موجود است:")
            return ADD_FIELD
        
        fields.append(new_field)
        
        if save_fields(fields):
            # اضافه کردن ستون جدید به فایل Excel موجود
            ...
            logger.info(f"User {update.effective_user.id} added field: {new_field}")
        else:
            await update.message.reply_text("❌ خطا در ذخیره فیلد.", reply_markup=get_keyboard())
...
async def delete_field_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """شروع حذف فیلد"""
    fields = load_fields()
    
    if len(fields) <= 1:
        await update.message.reply_text("❌ نمی‌توان همه فیلدها را حذف کرد. حداقل یک فیلد باید باقی بماند.")
        return FIELD_MANAGEMENT
    ...
    await update.message.reply_text(
        "🗑️ **کدام فیلد حذف شود؟**\n⚠️ توجه: تمام داده‌های این فیلد پاک خواهد شد!",
        reply_markup=ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
    )
    return DELETE_FIELD
...
async def delete_field_process(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if len(fields) <= 1:
            await update.message.reply_text("❌ نمی‌توان آخرین فیلد را حذف کرد.")
            return DELETE_FIELD
        ...
        if save_fields(fields):
            ...
            logger.info(f"User {update.effective_user.id} deleted field: {field_to_delete}")
        else:
...
async def show_fields(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = f"📋 **فیلدهای موجود ({len(fields)} فیلد):**\n\n"
    for i, field in enumerate(fields, 1):
        msg += f"  {i}. {field}\n"
...
async def reset_fields(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """بازگشت به فیلدهای پیش‌فرض"""
    try:
        if save_fields(DEFAULT_FIELDS):
            await update.message.reply_text(
                "🔄 **فیلدها به حالت پیش‌فرض بازگشت داده شد!**\n"
                f"📊 {len(DEFAULT_FIELDS)} فیلد پیش‌فرض بارگذاری شد.",
                reply_markup=get_keyboard()
            )
            logger.info(f"User {update.effective_user.id} reset fields to default")
        else:
            await update.message.reply_text("❌ خطا در بازگشت به پیش‌فرض.", reply_markup=get_keyboard())
        
    except Exception as e:
        logger.error(f"Error resetting fields: {e}")
        await update.message.reply_text("❌ خطا در بازگشت به پیش‌فرض.", reply_markup=get_keyboard())
    
    return ConversationHandler.END


async def change_theme(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """تغییر تم رنگی"""
    current_theme = load_user_theme(update.effective_user.id)
    
    keyboard = []
    for key, theme in THEMES.items():
        status = "✅" if key == current_theme else ""
        keyboard.append([InlineKeyboardButton(f"{theme['name']} {status}", callback_data=f"theme_{key}")])
    
    await update.message.reply_text(
        f"🎨 **انتخاب تم رنگی**\n"
        f"🔘 تم فعلی: {THEMES[current_theme]['name']}",
    ...
    if query.data.startswith("theme_"):
        ...
        if save_user_theme(user_id, theme):
            ...
            await query.edit_message_text(
                f"✅ **تم {THEMES[theme]['name']} اعمال شد!**\n"
                f"🎨 فایل Excel با رنگ‌بندی جدید آماده است."
            )
            logger.info(f"User {user_id} changed theme to {theme}")
        else:
            await query.edit_message_text("❌ خطا در تغییر تم.")
...
async def show_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # حجم فایل
        size_str = get_file_size_string(EXCEL_FILE)
...
🎨 **تنظیمات شما:**
  • تم فعلی: {THEMES[user_theme]['name']}
...
async def confirm_delete_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [KeyboardButton("⚠️ بله، همه را حذف کن"), KeyboardButton("❌ لغو")]
    ]
    
    await update.message.reply_text(
        "⚠️ **هشدار!**\n\n"
        "آیا مطمئن هستید که می‌خواهید **همه رکوردها** را حذف کنید؟\n"
...
async def delete_all_process(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """پردازش حذف همه رکوردها"""
    text = update.message.text
    
    if text == "⚠️ بله، همه را حذف کن":
        try:
            # حذف فایل Excel
            ...
            await update.message.reply_text(
                ...
                "🗂️ فایل Excel جدید و خالی ایجاد شد.",
                reply_markup=get_keyboard()
            )
            
            logger.info(f"User {update.effective_user.id} deleted all records")
...
async def show_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """نمایش راهنما"""
    help_text = """ℹ️ **راهنمای ربات Excel**

🔹 **عملیات اصلی:**
  • ➕ اضافه کردن: افزودن رکورد جدید
  • 📋 نمایش همه: مشاهده تمام رکوردها  
  • ✏️ ویرایش: تغییر اطلاعات موجود
  • 🗑️ حذف: پاک کردن رکورد خاص
  • 🔍 جستجو: پیدا کردن رکورد
...
🔹 **اطلاعات:**
  • 📊 آمار: نمایش اطلاعات سیستم
  • ℹ️ راهنما: این پیام

🔹 **نکات مهم:**
  • 🔢 فیلدهای عددی فقط عدد می‌پذیرند
  • 🎨 تم‌های مختلف برای زیباسازی Excel
  • 💾 تمام تغییرات خودکار ذخیره می‌شود
...
async def handle_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "➕ اضافه کردن":
        ...
        return await field_management_start(update, context)
    elif text == "🎨 تغییر تم":
        await change_theme(update, context)
    elif text == "📊 آمار":
        await show_stats(update, context)
    elif text == "🧹 حذف همه":
        return await confirm_delete_all(update, context)
    elif text == "ℹ️ راهنما":
...
def main():
    ...
    if TOKEN == "YOUR_BOT_TOKEN_HERE":
        print("❌ خطا: لطفاً توکن ربات را در فایل config.py وارد کنید")
        print("📝 فایل config.py را باز کرده و TOKEN را جایگزین کنید")
        return
    ...
    add_record_handler = ConversationHandler(
        ...
        states={
            ADD_DATA: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_record_process)],
        },
    ...
    delete_record_handler = ConversationHandler(
        ...
        states={
            DELETE_ROW: [MessageHandler(filters.TEXT & ~filters.COMMAND, delete_confirm)],
        },
        fallbacks=[CommandHandler("cancel", cancel), MessageHandler(filters.Regex("^❌ لغو$"), cancel)],
    )
    ...
    search_handler = ConversationHandler(
        ...
        states={
            SEARCH_FIELD: [MessageHandler(filters.TEXT & ~filters.COMMAND, search_field_select)],
            SEARCH_VALUE: [MessageHandler(filters.TEXT & ~filters.COMMAND, search_process)],
        },
        fallbacks=[CommandHandler("cancel", cancel), MessageHandler(filters.Regex("^❌ لغو$"), cancel)],
    )
    
    # ConversationHandler برای مدیریت فیلدها
    field_management_handler = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex("^⚙️ مدیریت فیلدها$"), field_management_start)],
        states={
            FIELD_MANAGEMENT: [MessageHandler(filters.TEXT & ~filters.COMMAND, field_management_handle)],
            ADD_FIELD: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_field_process)],
            DELETE_FIELD: [MessageHandler(filters.TEXT & ~filters.COMMAND, delete_field_process)],
        },
        fallbacks=[CommandHandler("cancel", cancel), MessageHandler(filters.Regex("^❌ لغو$"), cancel)],
    )
    ...
    delete_all_handler = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex("^🧹 حذف همه$"), confirm_delete_all)],
        states={
            CONFIRM_DELETE_ALL: [MessageHandler(filters.TEXT & ~filters.COMMAND, delete_all_process)],
        },
        fallbacks=[CommandHandler("cancel", cancel), MessageHandler(filters.Regex("^❌ لغو$"), cancel)],
    )
    ...
    print("✅ همه عملکردها فعال است:")
    print("   • اضافه/ویرایش/حذف رکورد")
    print("   • جستجو پیشرفته")
    print("   • مدیریت فیلدها")
    print("   • تم‌های رنگی")
    print("   • خروجی Excel زیبا")
